...
#include <gsplat/cuda/include/types.cuh>
#include <gsplat/cuda/include/2dgs.cuh>
...
__global__ void intersects_visibility_fwd_2dgs_skinny_outputs_kernel(
    ...
    const S *__restrict__ normals_camera,     // [C, N, 3] or [nnz, 3]                  // The normals in camera space.
    ...
    const bool *__restrict__ gaussian_masks, // [C, N] or [nnz]                  // Optional Gaussian mask to skip rendering specific gaussians.
    ...
    int32_t *__restrict__ visibility_indices, // [C, image_height, image_width, max_hits]  // Stores the index of the last Gaussian that contributed to each pixel.
    S *__restrict__ visibility_values // [C, image_height, image_width, max_hits]    // Stores the index of the Gaussian that contributes to the median depth for each pixel (bring over 0.5).
    ...
    visibility_indices += camera_id * image_height * image_width * max_hits;
    visibility_values += camera_id * image_height * image_width * max_hits;
    ...
    for (uint32_t b = 0; b < num_batches; ++b) {
        ...
        for (uint32_t t = 0; (t < batch_size) && !done; ++t) {
            ...
            if (gaussian_masks != nullptr && gaussian_masks[g] == false) {
                // this cuz gaussian contribute nothing to the pixel, but still attenuate
                vis = 0.f;
            }
            ...
            int32_t vis_idx = b * block_size + t;
            if (inside) {
                if (vis_idx < max_hits) {
                    visibility_indices[pix_id * max_hits + vis_idx] = g;
                    visibility_values[pix_id * max_hits + vis_idx] = vis;
                } else {
                    // go over all the hits and find the smallest one
                    int32_t min_idx = 0;
                    S min_val = visibility_values[pix_id * max_hits];
                    for (uint32_t k = 1; k < max_hits; ++k) {
                        S val = visibility_values[pix_id * max_hits + k];
                        if (val < min_val) {
                            min_val = val;
                            min_idx = k;
                        }
                    ...
                    if (vis > min_val) {
                        visibility_indices[pix_id * max_hits + min_idx] = g;
                        visibility_values[pix_id * max_hits + min_idx] = vis;
                    }
                }
            }
            ...
            // const S *n_ptr = normals_camera + g * 3;
            // GSPLAT_PRAGMA_UNROLL
...
call_kernel_with_dim(
    ...
    const torch::Tensor &normals_camera,   // [C, N, 3]
    const at::optional<torch::Tensor> &backgrounds, // [C, channels]
    const at::optional<torch::Tensor> &masks, // [C, tile_height, tile_width]
    const at::optional<torch::Tensor> &gaussian_masks, // [C, N] or [nnz]
    // image size
    const uint32_t image_width,
    const uint32_t image_height,
    const uint32_t tile_size,
    ...
    GSPLAT_CHECK_INPUT(normals_camera);
    ...
    torch::Tensor visibility_indices = torch::full(
        {C, image_height, image_width, max_hits}, -1,
        means2d.options().dtype(torch::kInt32)
    );
    torch::Tensor visibility_values = torch::zeros(
        {C, image_height, image_width, max_hits},
    ...
    if (cudaFuncSetAttribute(
            intersects_visibility_fwd_2dgs_skinny_outputs_kernel<CDIM, float>,
    ...
    intersects_visibility_fwd_2dgs_skinny_outputs_kernel<CDIM, float>
        <<<blocks, threads, shared_mem, stream>>>(
            ...
            normals_camera.data_ptr<float>(),
            backgrounds.has_value() ? backgrounds.value().data_ptr<float>()
                                    : nullptr,
            masks.has_value() ? masks.value().data_ptr<bool>() : nullptr,
            gaussian_masks.has_value() ? gaussian_masks.value().data_ptr<bool>() 
                                       : nullptr,
            image_width,
            image_height,
            tile_size,
            ...
            max_hits,
            visibility_indices.data_ptr<int32_t>(),
            visibility_values.data_ptr<float>()
        );
...
std::tuple<
    torch::Tensor,
    torch::Tensor>
intersects_visibility_fwd_2dgs_skinny_outputs_tensor(
    // Gaussian parameters
    const torch::Tensor &means2d,   // [C, N, 2] or [nnz, 2]
    const torch::Tensor &ray_transforms,    // [C, N, 3] or [nnz, 3]
    const torch::Tensor &colors,    // [C, N, channels] or [nnz, channels]
    const torch::Tensor &opacities, // [C, N]  or [nnz]
    const torch::Tensor &normals_camera,   // [C, N, 3] or [nnz, 3]
    const at::optional<torch::Tensor> &backgrounds, // [C, channels]
    const at::optional<torch::Tensor> &masks, // [C, tile_height, tile_width]
    const at::optional<torch::Tensor> &gaussian_masks, // [C, N] or [nnz]
    // image size
