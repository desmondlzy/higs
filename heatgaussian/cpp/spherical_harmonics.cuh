...
__host__ __device__ inline unsigned num_sh_degree(const unsigned n_bases) {
    if (n_bases == 1)
        return 0;
    if (n_bases == 4)
        return 1;
...
__device__ inline void eval_sh(
...
__device__ inline void eval_sh_grad(
    const unsigned degree,
	const unsigned num_channels,
    const float3 &dir,
...
__global__ inline void eval_sh_forward_kernel(
...
__global__ inline void eval_sh_backward_kernel(
    const unsigned num_points,
    const unsigned num_channels,
    const unsigned degree,
    const unsigned degrees_to_use,
