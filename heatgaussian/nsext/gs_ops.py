...
def split(
	...
	def param_fn(name: str, p: Tensor) -> Tensor:
		...
		elif name == "thermal_opacities" and revised_opacity:
			new_thermal_opacities = 1.0 - torch.sqrt(1.0 - torch.sigmoid(p[sel]))
			p_split = torch.logit(new_thermal_opacities).repeat(repeats)  # [2N]
		else:
...
def reset_opa(
	...
	_update_param_with_optimizer(
		param_fn, optimizer_fn, params, optimizers, names=["opacities", "thermal_opacities"]
	)
...
def relocate(
	...
	thermal_opacities = torch.sigmoid(params["thermal_opacities"])
	...
	new_opacities, new_rgb_scales = compute_relocation(
		opacities=opacities[sampled_idxs],
		scales=torch.exp(params["scales"])[sampled_idxs],
		ratios=torch.bincount(sampled_idxs)[sampled_idxs] + 1,
		binoms=binoms,
	)
	new_thermal_opacities, new_thermal_scales = compute_relocation(
		opacities=thermal_opacities[sampled_idxs],
		scales=torch.exp(params["thermal_scales"])[sampled_idxs],
		ratios=torch.bincount(sampled_idxs)[sampled_idxs] + 1,
		binoms=binoms,
	)
	new_opacities = torch.clamp(new_opacities, max=1.0 - eps, min=min_opacity)
	new_thermal_opacities = torch.clamp(new_thermal_opacities, max=1.0 - eps, min=min_opacity)
	new_scales = (new_rgb_scales + new_thermal_scales) / 2

	def param_fn(name: str, p: Tensor) -> Tensor:
		if name == "opacities":
			p[sampled_idxs] = torch.logit(new_opacities)
		elif name == "thermal_opacities":
			p[sampled_idxs] = torch.logit(new_thermal_opacities)
		elif name == "scales":
